...
from .models import AIConfig, AIModel, AIProvider, KnowledgeChunk, KnowledgeDocument, KnowledgeBase, AIPromptTemplate
...
class RAGService:
    ...
    def _get_prompt(self, code: str) -> str:
        ...
        try:
            ...
            logger.warning(f"[RAG] Failed to load prompt '{code}' from database: {e}. Falling back to default.")
    ...
    def get_chat_chain(self, history_id: int = None, auth_context: dict = None):
        ...
        template = self._get_prompt("rag_chat")
    ...
    def diagnose_log(self, log_content: str, context_info: dict, auth_context: dict = None):
        ...
        template = self._get_prompt("log_diagnosis")
        prompt = ChatPromptTemplate.from_template(template)
    ...
    def diagnose_alert(self, query: str):
        ...
        template = self._get_prompt("alert_diagnosis")
        prompt = ChatPromptTemplate.from_template(template)
        # 封装同步执行链
        chain = (
    ...
    def generate_dag(self, prompt_text: str, context_data: dict = None):
        template = self._get_prompt("dag_generation")
        prompt = ChatPromptTemplate.from_template(template)
    ...
    def refine_dag(self, prompt_text: str, nodes: list, edges: list, auth_context: dict = None):
        ...
        template = self._get_prompt("dag_refine")
        prompt = ChatPromptTemplate.from_template(template)
        chain = prompt | self.llm | StrOutputParser()
    ...
    def explain_pipeline(self, nodes: list, edges: list):
        template = self._get_prompt("pipeline_explain")
        prompt = ChatPromptTemplate.from_template(template)
        chain = prompt | self.llm | StrOutputParser()
