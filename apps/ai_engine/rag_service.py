...
class RAGService:
    ...
    def rewrite_query(self, query: str):
        ...
        template = """你是一个专业的 SRE 技术搜索优化专家。
请将用户的问题改写为一个更适合在技术文档知识库中进行语义检索的查询语句。

【改写原则】
1. **保留原始术语**：严禁修改或翻译特定的技术缩写、命令名、配置项（如：k8s, redis, nginx, yaml, pod, deployment 等）。
2. **术语扩充**：如果用户使用了缩写，请在改写后的语句中同时包含缩写和全称，采用“扩充”而非“替换”的策略。例如：用户输入 'k8s'，应改写包含 'Kubernetes (k8s)'。
3. **去除干扰**：去掉提问中的口语化修饰词（如“请问”、“我想知道”）。
4. **意图清晰**：明确用户想要查找的资产类型或操作。
5. **只返回改写结果**：不要有任何解释。
