...
<p align="left">
  <a href="https://ansflow.cyfee.com"><img src="https://img.shields.io/badge/Python-3.11+-3776AB?style=for-the-badge&logo=python&logoColor=white" alt="Python 3.11+"></a>
  ...
  <a href="https://ansflow.cyfee.com"><img src="https://img.shields.io/badge/Docker-支持-2496ED?style=for-the-badge&logo=docker&logoColor=white" alt="Docker"></a>
...
- **混合检索与重排**：结合向量检索与 BM25 关键词检索，并引入 **BGE-Reranker-base** 模型进行二次重排序，显著提升回答的相关性。
- **查询改写**：利用 LLM 对用户原始问题进行语义改写与扩展，解决表述不精准导致的检索失效问题。
- **知识闭环**：支持将精彩问答或诊断结论一键存入 RAG 系统，实现经验实时沉淀。
- **物理同步**：完善的文档生命周期管理，删除文档时自动物理删除原始文件并彻底清理向量索引。
- **语义缓存**：引入高置信度向量检索，对已解决过的重复问题实现毫秒级“秒回”。
- **语义问答**：基于本地运维手册进行向量化存储，支持精准检索。
- **对话分类**：自动区分“故障诊断”与“智能对话”记录，支持标题搜索。
- **智能摄取流水线**：
  - **多模式路由**：支持原生文本(Native)、视觉OCR、及混合增强(Hybrid)三种解析模式，灵活应对不同格式。
  - **提示词引导**：支持在上传文档时输入解析 Prompt（如：请按表格提取），精准干预 OCR 提取质量。
  - **自动清洗管道**：内置标准化清洗逻辑，自动修复 OCR 产生的杂断行、空白符及特殊干扰项。
  - **Context-Aware 切片**：自动感知模型 `num_ctx`（上下文窗口），动态计算最优切片步长，兼顾信息完整度与检索冗余。
  - **多维能力模型**：支持模型同时具备 LLM/Vision/Embedding/Rerank 等多项能力，并支持全局默认 OCR 视觉模型的精细化分发。
- **WebSocket 流式交互**：基于 Django Channels 提供全链路打字机式实时流式对话，优化等待反馈体验。

### 2. 任务脉冲监控 (SRE Center)
- **实时状态**：高频监控异步任务的执行健康度与处理趋势。
- **脉冲心跳**：后台常驻监控 (`pulse_monitor`) 实时追踪任务吞吐量与失败率。
...
- **Webhook 鉴权加固**：新增告警接收端点基于 Token 的安全校验机制。支持通过配置中心设置 `webhook_token`（空值时自动向下放行兼容）。可通过 HTTP 请求头（`Authorization: Bearer <token>`）或 URL 参数（`?token=<token>`）进行鉴权校验。
- **AI 自愈熔断保护**：在自愈处理任务中引入告警指纹频次判定。触发熔断后将自愈流程转为待审批工单（`awaiting_approval`），并屏蔽该告警指纹的自动触发，防止发生自愈无限死循环。

### 4. AIGC 意图编排 (Pipeline Gen)
- **自然语言编排**：用户输入需求文字，AI 自动生成符合 ReactFlow 规范的 DAG JSON 结构并反馈给前端画布。

### 5. 配置中心与安全合规 (Config Center & Compliance)
- **自定义 AI 提示词模板**：支持对 RAG 问答、日志诊断、告警分析、DAG 生成与优化、流水线解释及 OCR 视觉解析等 7 大核心场景提示词模板的动态管理与安全占位符校验，具备代码级默认值容错回退保护。
- **多渠道通知配置**：集成飞书、钉钉机器人通知，支持图形化全局开关、级别控制、细粒度事件白名单筛选（流水线启动/结果、审批挂起/放行、任务运行完毕等）及环境变量兜底。
- **等保 2.0 安全合规基线**：深度对接国家网络安全等级保护 2.0（等保三级）技术指标，实现主机的全量合规巡检评分与漏洞剧本级联修复。

### 6. 多租户隔离与跨项目资产共享 (Project Multi-Tenancy & Asset Sharing)
- **物理与逻辑隔离**：引入多项目/工作区（`Project` 和 `ProjectMember`）机制，对主机、通用凭据、流水线、K8s 集群、Ansible 任务及自愈策略等核心资产实现基于项目的行级过滤隔离。
- **跨项目授权共享 (ProjectAssetShare)**：支持项目所有者将特定资产以只读（`read`）、引用执行（`use`，可在流水线中跨项目调用但不能查看敏感密码）和完全控制（`full`）三级权限，安全定向授权给目标项目使用。

### 7. 多维运营数据报表 (system_reports)
- **多维指标数据聚合**：后端通过高性能 SQL 聚合查询，统计系统内历史告警自愈频次、Celery 脉冲健康度、各流水线执行成功率及耗时，以及主机安全基线评分历史记录。
- **异步报表导出引擎**：后台通过 Celery 调度导出任务，在 Worker 端异步拉取底层多维表，并在通知中心为用户生成压缩的 CSV 数据报表下载链接。
- **行级租户隔离校验**：统计数据完美对接 SmartRBAC 权限约束，隔离项目可见性，确保报表只在授权项目范围内可见。
